...
public class Missile : MonoBehaviour
{
    ...
    void Update()
    {
        if (m_tfTarget != null)
        {
            ...
            Vector3 t_dit = (m_tfTarget.position - transform.position).normalized;//거리가 의미없기때문에 방향만 가져옴
            transform.up = Vector3.Lerp(transform.up, t_dit, 0.25f);
        }
    }

    private void OnCollisionEnter(Collision collision)
    {
        if (collision.transform.CompareTag("EnemyCube"))
        {
